...
class TMPS:
    ...
    def TMPS_anchor_process(self, content_image, target, epoch):
        ...
        # if self.args.mode is not None and self.args.mode not in ['only_dir', 'add_l1']:
        if epoch <= 20:
            source_top_indice = source_top_indice[source_cos_sim[source_top_indice] > 0.9]
                # source_top_indice = source_cos_sim.topk(source_top_indice.size(0)//2).indices
        ...
        img_np = target.squeeze().cpu().detach().numpy()
        img_np = np.transpose(img_np, (1, 2, 0))
        img_np = (img_np * 255).astype(np.uint8).copy()
        img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
        
        img_tr = target.clone().requires_grad_(True).to(self.device)
        img_tr = img_tr.squeeze().cpu().detach().numpy()
        img_tr = np.transpose(img_tr, (1, 2, 0))
        img_tr = (img_tr * 255).astype(np.uint8).copy()
        img_tr = cv2.cvtColor(img_tr, cv2.COLOR_RGB2BGR)
        img_tr_mask = img_tr.copy()
        # white image
        img_wt = np.ones_like(img_np) * 255
        img_src = content_image.squeeze().cpu().detach().numpy()
        img_src = np.transpose(img_src, (1, 2, 0))
        img_src = (img_src * 255).astype(np.uint8).copy()
        img_src = cv2.cvtColor(img_src, cv2.COLOR_RGB2BGR)
        img_src_mask = img_src.copy()
        for box in pos_tensor_50:
            x, y, h, w = box
            cv2.rectangle(img_np, (int(x), int(y)), (int(x+w), int(y+h)), (0, 255, 0), 2)  # Green color, 2px thickness
    ...
    def TMPS_process(self, content_image, target, epoch):
        ...
        #     source_crop, pos = random_square_crop(content_image, True)
        #     source_crop_ = self.augment(source_crop)
        #     source_img_proc.append(source_crop_)
        #     pos_list.append(pos)
        # source_img_proc = torch.cat(source_img_proc, dim=0)
        # pos_tensor = torch.tensor(pos_list).to(self.device)
        source_img_aug = source_img_proc
        source_features_ = self.clip_model.encode_image(utils.clip_normalize(source_img_aug, self.device))
        source_features_ /= (source_features_.clone().norm(dim=-1, keepdim=True))
        source_cos_sim = F.cosine_similarity(source_features_, self.text_source.repeat(source_features_.size(0), 1), dim=1)
        source_top_indice_ = source_cos_sim.topk(3).indices
        source_top_image_features = source_features_[source_top_indice_]
        ...
        pos_tensor_50 = pos_tensor[source_top_indice]
        if self.args.loss == 'ours_l1':
            target_loss_img = target.clone().to(self.device)
        # print(pos_tensor_50.shape, 'pos_tensor_50.shape')
        if len(pos_tensor_50.shape) == 1 and pos_tensor_50.shape[0] == 4:
            pos_tensor_50 = pos_tensor_50.unsqueeze(0)
        for n in range(pos_tensor_50.size(0)):
        ...
        if self.args.source_patch_sel:
            output['source_patch_features'] = source_features_[source_top_indice]
        output['image_features'] = image_features
        img_np = target_loss_img.squeeze().cpu().detach().numpy()
        img_np = np.transpose(img_np, (1, 2, 0))
        img_np = (img_np * 255).astype(np.uint8).copy()
        img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
    ...
    def TMPS_old_process(self, content_image, target, epoch):
        output = {}
        source_img_proc = []
        pos_list = []
        img_proc = []
        ...
        for n in range(pos_tensor_50.size(0)):
            ...
            if pos.shape[0] == 3:
                ...
                y, x, h, w = pos[0], pos[1], pos[2], pos[3]
            target_crop = crop(target, y, x, h, w)
            target_crop_ = self.augment(target_crop)
            img_proc.append(target_crop_)
            if self.args.loss == 'ours_l1':
        ...
        if epoch % 10 == 0:
            ...
            # cv2.imwrite(os.path.join(self.args.save_dir, 'target_img_patch{}.jpg'.format(epoch)), img_tr)
            cv2.imwrite(os.path.join(self.args.save_dir, 'target_loss_tmps{}.jpg'.format(epoch)), img_loss_np)
            # img_wt = cv2.cvtColor(img_wt, cv2.COLOR_RGB2BGR)
            # cv2.imwrite(os.path.join(self.args.save_dir, 'target_loss_tmps_wt{}.jpg'.format(epoch)), img_wt)
            # cv2.imwrite(os.path.join(self.args.save_dir, 'source_img_patch{}.jpg'.format(epoch)), img_src)
            # cv2.imwrite(os.path.join(self.args.save_dir, 'source_img_mask{}.jpg'.format(epoch)), img_src_mask)
            # cv2.imwrite(os.path.join(self.args.save_dir, 'target_img_mask{}.jpg'.format(epoch)), img_tr_mask)
    ...
    def select_anchor_patches_(self, image, top_k_indices, epoch, strides=64, grid_size=9):
        patches = []
        bbox_list = []
        ratio = 1
        ...
        for num in range(self.num_crops):
            ...
            i = top_k_indices[random.randint(0, top_k_indices.size(0) - 1)]
            grid_y = i.item() // grid_size
            grid_x = i.item() % grid_size

            # Select a random center within the grid cell
            center_y = random.randint(grid_y * strides, min((grid_y + 1) * strides, image.shape[2]))
            center_x = random.randint(grid_x * strides, min((grid_x + 1) * strides, image.shape[3]))

            # Calculate the maximum possible crop size based on the grid size
            max_crop_height = min(center_y, image.shape[2] - center_y, int(64*ratio), int(128*ratio)) * 2
            max_crop_width = min(center_x, image.shape[3] - center_x, int(64*ratio), int(128*ratio)) * 2

            crop_height = random.randint(int(64*ratio), max_crop_height)
            crop_width = random.randint(int(64*ratio), max_crop_width)
            ...
            patch_ = self.augment(patch)
            patches.append(patch_)
            bbox_list.append([x, y, crop_height, crop_width])
        patches = torch.cat(patches, dim=0)
        bbox_list = torch.tensor(bbox_list).to(self.device)
        return patches, bbox_list
